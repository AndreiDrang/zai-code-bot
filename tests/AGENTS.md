...
├── handlers/                    # Handler-focused test cases
│   ├── ask.test.js
│   ├── explain.test.js
│   ├── impact.test.js
│   ├── review.test.js
...
└── integration/                 # End-to-end command/review pipeline checks
    ├── AGENTS.md
    ├── command-pipeline.test.js
    ├── pr-auto-review.test.js
    └── fixtures/
        └── events.js
```
...
- End-to-end schedule event → context → Z.ai mock → validated PR is still a gap; unit coverage of the grounded flow exists via the `handleUpdateAgentsTask` seam (`__callZaiForTest`).
