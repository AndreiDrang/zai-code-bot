...
describe('storage key contracts', () => {
  ...
  it('builds deterministic pr-card (KV) and pr-context (R2) keys', () => {
    ...
    expect(prContextKey(10, 7, 'manifest')).toBe('v2/prs/10/7/context/manifest.json');
    expect(prContextKey(10, 7, 'description')).toBe('v2/prs/10/7/context/description.md');
    expect(prContextDiffKey(10, 7, 'src/cache.ts')).toBe(
  ...
  it('builds a per-command result key under the same /context/ prefix (overwrite)', () => {
    ...
    expect(prCommandResultKey(10, 7, 'review')).toBe('v2/prs/10/7/context/review.md');
    expect(prCommandResultKey(10, 7, 'describe')).toBe('v2/prs/10/7/context/describe.md');
  });
  ...
  it('rejects unsafe key components and unknown context kinds', () => {
    ...
    expect(() => prContextKey(10, 7, 'diff')).toThrow('Invalid PR context kind');
    expect(() => prContextDiffKey(10, 7, '../secret')).toThrow('repository-relative path');
