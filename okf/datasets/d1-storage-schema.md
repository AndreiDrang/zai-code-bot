...
description: The nine authoritative D1 tables (migration 0001 + hardening 0002) that hold all job, delivery, run, artifact, publication, and configuration state.
source_paths:
...
D1 (`bot-db`) is the single source of truth. The schema is defined across two
...
| `pull_requests` | `(repository_id, pr_number)` | Latest known PR state (head SHA, title, author) |
